...
if [ "$#" -lt 6 ] || [ "$#" -gt 7 ]; then
    printf 'usage: %s RSOMICS_BAM SAMTOOLS INPUT OUTPUT_DIR ROUNDS THREADS [md|embedded]\n' "$0" >&2
    exit 2
fi
...
case "$mode" in
    ...
    *)
        printf 'invalid reference mode: %s\n' "$mode" >&2
        exit 2
        ;;
esac
...
"$ours" reference --quiet $extra -@ "$threads" "$input" > "$output/rsomics.fa"
"$oracle" reference --quiet $extra -@ "$threads" "$input" > "$output/samtools.fa"
cmp "$output/rsomics.fa" "$output/samtools.fa"

{
    ...
    shasum -a 256 "$ours" "$oracle" "$input" "$0" "$output/rsomics.fa" "$output/samtools.fa"
} > "$environment"
...
"$ours" reference --quiet $extra -@ "$threads" "$input" > /dev/null
"$oracle" reference --quiet $extra -@ "$threads" "$input" > /dev/null

round=1
while [ "$round" -le "$rounds" ]; do
    if [ $((round % 2)) -eq 1 ]; then
        measure rsomics "$round" "$ours" reference --quiet $extra -@ "$threads" "$input"
        measure samtools "$round" "$oracle" reference --quiet $extra -@ "$threads" "$input"
    else
        measure samtools "$round" "$oracle" reference --quiet $extra -@ "$threads" "$input"
        measure rsomics "$round" "$ours" reference --quiet $extra -@ "$threads" "$input"
    fi
