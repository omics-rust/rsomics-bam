...
impl Ord for HeapEntry {
    fn cmp(&self, other: &Self) -> Ordering {
        compare_entries(self.order, &self.entry, &other.entry)
    ...
    W: Write + Send + 'static,
{
    ...
    let mut ingest = |record| {
        ...
        let entry = make_entry(record, options.order, &header, &libraries, ordinal)?;
        let entry_memory = entry_memory(&entry)?;
        if !entries.is_empty()
            && memory
...
fn sort_entries(entries: &mut [Entry], order: Order, pool: &rayon::ThreadPool) {
    pool.install(|| {
        entries.par_sort_unstable_by(|a, b| {
            compare_entries(order, a, b).then_with(|| a.ordinal.cmp(&b.ordinal))
        });
    });
}
    ...
    W: Write + Send + 'static,
{
    ...
    for (run, reader) in readers.iter_mut().enumerate() {
        if let Some(record) = reader.next()? {
            heap.push(HeapEntry {
                entry: make_entry(record, order, header, libraries, 0)?,
                order,
                run,
                sequence: 0,
            });
    ...
    while let Some(item) = heap.pop() {
        ...
        if let Some(record) = readers[item.run].next()? {
            heap.push(HeapEntry {
                entry: make_entry(record, order, header, libraries, sequence)?,
                order,
                run: item.run,
                sequence,
            });
...
fn entry_memory(entry: &Entry) -> Result<u64> {
    let payload = u64::try_from(entry.record.as_bytes().len())
    ...
    payload
        .checked_add(mem::size_of::<Entry>() as u64)
        .and_then(|value| value.checked_add(key))
        .ok_or_else(|| RsomicsError::InvalidInput("sort memory accounting overflows".to_owned()))
}
...
fn compare_entries(order: Order, a: &Entry, b: &Entry) -> Ordering {
...
fn library_lookup(header: &sam::Header) -> HashMap<Vec<u8>, Arc<[u8]>> {
    header
        .read_groups()
        .iter()
...
fn set_sort_order(header: &mut sam::Header, order: Order) {
    let hd = header
        .header_mut()
