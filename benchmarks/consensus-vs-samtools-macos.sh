...
repeats=${5:-20}
threads=${6:-0}
ours_output=$output_dir/rsomics.fa
samtools_output=$output_dir/samtools.fa
...
run_ours() {
    "$ours" consensus --threads "$threads" -o "$ours_output" "$input"
}

run_samtools() {
    "$samtools" consensus --threads "$threads" -o "$samtools_output" "$input"
}
...
measure() {
    ...
    if [[ $tool == rsomics ]]; then
        /usr/bin/time -p -l -o "$timing" "$ours" consensus \
            --threads "$threads" -o "$ours_output" "$input"
    else
        /usr/bin/time -p -l -o "$timing" "$samtools" consensus \
            --threads "$threads" -o "$samtools_output" "$input"
    fi
