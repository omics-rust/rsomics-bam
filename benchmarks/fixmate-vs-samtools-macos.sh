...
run_ours() {
    local round=$1
    local order=$2
    local command=("$ours" fixmate --no-pg -m)
...
run_samtools() {
    local round=$1
    local order=$2
    /usr/bin/time -p -l -o "$timing" "$samtools" fixmate -z off --no-PG -m \
...
ours_command=("$ours" --json fixmate --no-pg -m)
if [[ $mode == equal-workers ]]; then
    ours_command+=(-@ 4)
fi
ours_command+=("$input" -o "$ours_output")
"${ours_command[@]}" > "$output_dir/rsomics-summary.json"
"$samtools" fixmate -z off --no-PG -m -@ "$samtools_threads" \
